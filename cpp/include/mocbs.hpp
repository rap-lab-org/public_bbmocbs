...
struct CBSConflict {
  ...
  inline bool IsValid() {return !(ta == -1); };
  inline bool IsNodeCft() {return (ta == tb); } ;
  std::string ToStr() ;
};
/**
 *
...
/**
 * @brief Tell if two conflicts are the same or not.
 */
...
struct CBSNode {
  ...
  PathSet times; // @2022-04-03, newly added, the corresponding times of nodes in "paths".
                 // Regular MO-CBS doesn't use it.
...
public:
  ...
  virtual ~MOCBS_TB();
  /**
   * @brief an API for python wrapper. only for grid like world. 
   * Add to make it compatible with pybind11.
   * NOTE: Input wait_cost is of format std::vector<long>&, to make it easy for python usage.
   */
...
protected:
   ...
   * 
   */
  virtual void _CountDupConflict();
  ...
  std::unordered_map<int, basic::GridkConn> _tempAgentGrids; // temp fix, to get pybind11 work.
  // basic::Graph* _graph; // workspace graph
  // basic::GridkConn temp_g; // temp fix, to get pybind11 work.
  size_t _cdim = 0;
  ...
  // search::Frontier _solFront; // for fast solution filtering.

  double _epsilon = 0.0; // @2021-10-01

  // Aux, count duplicated conflicts
  std::vector<CBSConflict> _cfts;
};


/**
 * @brief. Use EMOA* as the low-level planner, no diff between #obj anymore.
 */
class MOCBS_TEMOA : public MOCBS_TB {
public:
  MOCBS_TEMOA() ;
  virtual ~MOCBS_TEMOA() ;
protected:
