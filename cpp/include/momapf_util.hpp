 ...
 *
 */
typedef std::unordered_map<long, std::vector<long>> PathSet ;
/**
 *
 */
...
struct MOMAPFResult
{
  ...
  std::unordered_map< long, PathSet > times; // @2022-04-03 newly added, stores the corresponding times of path nodes.
  // It's empty for regular MO-CBS/MOM*.
  ...
  std::vector<double> others; // for customized usage.
};
