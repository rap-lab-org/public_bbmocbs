...
/*******************************************************************************************
 * NOTE to myself. @2021-10-19
 * look at the note in the header file moa.hpp
 *******************************************************************************************/
...
MOA::~MOA() {
  ...
  // std::cout << "MOA destructor done" << std::endl;
};

void MOA::SetGraphPtr(basic::Graph* g) {
  _graph = g;
  // if (_graph->GetCostDim() != 2) {
  //   std::cout << "[ERROR] MOA, Input graph has cost dim = " << _graph->GetCostDim() << std::endl;
  //   throw std::runtime_error("[ERROR] MOA, Not bi-objective !");
  // }
};
