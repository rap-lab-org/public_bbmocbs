...
std::string CBSConflict::ToStr(){
  return std::to_string(i) + "," + std::to_string(j) + "," + std::to_string(vi) + "," 
    + std::to_string(vj) + "," + std::to_string(ta) + "," + std::to_string(tb);
};
...
bool SameConflict(CBSConflict& cft1, CBSConflict& cft2)
{
  bool res = (cft1.i == cft2.i) && (cft1.j == cft2.j) && (cft1.vi == cft2.vi) 
             && (cft1.vj == cft2.vj) && (cft1.ta == cft2.ta) && (cft1.tb == cft2.tb);
  return res;
}; 
...
void MOCBS_TB::Search(std::vector<long>& starts, std::vector<long>& goals, double time_limit, double eps) {
  ...
  while ( true ) {
    ...
    _cfts.push_back(cft); // @2022-10-19 This is just to count duplicated conflicts.
  ...
  // count duplicated search. OK to remove. @2022-10-19
  _CountDupConflict();
...
void MOCBS_TB::_CountDupConflict() {
  int count = 0;
  std::unordered_set<std::string> unique_cft;
  for (int idx = 0; idx < _cfts.size(); idx++) { // loop over all conflicts
    unique_cft.insert(_cfts[idx].ToStr());
  }
  // add more statistics about duplicated search!
  _result.others.push_back(_cfts.size());
  _result.others.push_back(unique_cft.size());
  return ;
};
...
MOCBS_TEMOA::MOCBS_TEMOA() {
  // std::cout << "[INFO] Constructor - MOCBS_TN" << std::endl;
};

MOCBS_TEMOA::~MOCBS_TEMOA() {};

int MOCBS_TEMOA::_LsearchPlan(int ri, std::vector<CBSConstraint*>& cstrs, search::MOSPPResult* out)
{
  auto tnow = std::chrono::steady_clock::now();
  auto time_passed = std::chrono::duration<double>(tnow - _t0).count();
  double remain_time = _tlimit - time_passed;
  ...
  auto retFlag = RunEMOAstGrid(_tempAgentGrids[ri], _vo[ri], _vd[ri], remain_time, _agentCosts[ri], ncs, ecs, out ); 
  auto tnow2 = std::chrono::steady_clock::now();
  _result.rt_searchLow += std::chrono::duration<double>(tnow2 - tnow).count();
  _result.n_expandLow += out->n_expanded; // update low level expanded.
  return retFlag;
